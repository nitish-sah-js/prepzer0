      ...
      try {
        ...
        if (!user.userallowed) {
          if (user.usertype === "teacher") {
            return done(null, false, {
              message:
                "Your teacher account is pending administrator approval. You will be notified once approved.",
            })
          } else {
            return done(null, false, {
              message: "Verify your account to log in",
            })
          }
        }
...
app.post("/update-integrity", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Integrity event '${eventType}' updated.`,
    })
  } catch (error) {
