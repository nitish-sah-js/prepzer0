    ...
    for (let i = 0; i < students.length; i++) {
      ...
      try {
        ...
        if (!email || !usn || !department || !semester) {
          errors.push(
            `Line ${lineNumber}: Missing required fields (email, usn, department, or semester)`
          )
          skipCount++
          continue
        }
        ...
        if (semester < 1 || semester > 8 || isNaN(semester)) {
          errors.push(
            `Line ${lineNumber}: Semester must be between 1 and 8 - ${semester}`
          )
          skipCount++
          continue
        }
        ...
        if (existingUser) {
          errors.push(
            `Line ${lineNumber}: User already exists with email ${email} or USN ${usn}`
          )
          skipCount++
          continue
        }
        ...
        const imageurl =
          row.imageurl?.trim() || row.profilepicture?.trim() || undefined
