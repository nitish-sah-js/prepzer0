...
exports.trackUserActivity = async (req, res) => {
  ...
  try {
    const { examId, timestamp, status = "active" } = req.body
    const userId = req.user._id // SECURITY FIX: Use authenticated user's ID only
    ...
    if (existingRecord && existingRecord.isAllowedResubmit === true) {
      console.log(
        "Resubmit allowed: updating StartedAt and setting allowedResubmit to false"
      )
      updateData.startedAt = timestamp // Update startedAt time
      updateData.isAllowedResubmit = false // Set allowedResubmit to false
      console.log(
        "Resubmit detected: updating startedAt and setting allowedResubmit to false"
      )
    }
