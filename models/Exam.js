  ...
  try {
    ...
    if (!hasExcel) {
      ...
      if (!this.departments || this.departments.length === 0) {
        return next(
          new Error(
            "At least one department must be selected when not using Excel upload."
          )
        )
      }
      ...
      if (!this.semester) {
        return next(
          new Error("Semester must be specified when not using Excel upload.")
        )
      }
    }
...
ExamSchema.statics.validateExamData = function (
  examData,
  hasExcelCandidates = false,
